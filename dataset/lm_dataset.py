...
class SFTDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        try:
            # 关闭随机 system prompt，先保证学生手册数据处理稳定
            conversations = pre_processing_chat(
            ...
            if not isinstance(prompt, str):
                raise TypeError(f"prompt 不是字符串，而是 {type(prompt)}")

            # 更明确地得到一维 token id 列表
            ...
            if len(input_ids) > 0 and isinstance(input_ids[0], list):
                raise TypeError(f"input_ids 是二维列表，index={index}")

            if not all(isinstance(x, int) for x in input_ids):
                bad = [(i, type(x), x) for i, x in enumerate(input_ids) if not isinstance(x, int)]
                raise TypeError(f"input_ids 中存在非 int 元素，index={index}, bad={bad[:10]}")

            labels = self.generate_labels(input_ids)

            # 检查 labels
            if len(labels) > 0 and isinstance(labels[0], list):
                raise TypeError(f"labels 是二维列表，index={index}")

            if not all(isinstance(x, int) for x in labels):
                bad = [(i, type(x), x) for i, x in enumerate(labels) if not isinstance(x, int)]
                raise TypeError(f"labels 中存在非 int 元素，index={index}, bad={bad[:10]}")
